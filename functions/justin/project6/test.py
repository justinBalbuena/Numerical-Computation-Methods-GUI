            ...
            case 'true_MAE':
                ...
                for i in range(len(new_x)):
                    ...
                    for j in range(len(new_x)):
                        inner_sum += A[i, j] * new_x[j]  # Use original matrix A
                    inner_sum -= A[i, column_indices - 1]  # Subtract constant term
                    full_sum += abs(inner_sum)
                error = full_sum / row_indices
                print(error)
                if error < tolerance:
                    return new_x
            case 'true_RMSE':
                ...
                for i in range(len(new_x)):
                    ...
                    for j in range(len(new_x)):
                        inner_sum += A[i, j] * new_x[j]  # Use original matrix A
                    inner_sum -= A[i, column_indices - 1]  # Subtract constant term
                    full_sum += pow(inner_sum, 2)
                error = sqrt(full_sum / row_indices)
                print(error)
                if error < tolerance:
                    return new_x
...
while True:
    ...
    stopping_criteria = input("Please enter MAE, RMSE, true_MAE, or true_RMSE for your stopping criteria: ")
